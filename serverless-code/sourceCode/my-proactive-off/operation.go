...
func genRequest(ID int, FunctionID int, IngressID int, ArriveTime int, isNotPredicted bool, isNotUsed bool) (Request, error){
    ...
    r  = Request{ 
        ...
        IsNotUsed: isNotUsed,  //check if being used
    }
...
func genRequests ( sample []int, funcID int, timeSlot int, samplePred []int) {
    ...
    //sample is the vector of all nodes number
    

    for i :=0; i < len(sample); i++{
        // log.Printf("sample value %d \n", sample[i])
        difference := samplePred[i] - sample[i];
        ...
        for j := 0; j < sample[i]; j++{
            ...
            if difference < j{
                ...
                fmt.Printf(" isNotPredicted true")
            }
            
            if difference < 0 && float64(unused_count) <= math.Abs(float64(difference)){

                isNotUsed = true

                unused_count++

                fmt.Printf(" isNotUsed: true")
            ...
            r, _ := genRequest(requestCount_G, funcID, i, timeSlot, isNotPredicted, isNotUsed)
